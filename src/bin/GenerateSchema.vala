...
"""generate-schema.pl --dsn "<dsn>" --output </path/to/output> <options>
 ...
 --subclass  Generate subclasses to edit, so superclasses can be replaced later.
             Will generate into Entity/ and Implementation/. Future calls should
             output to output/Entity and without a loader.
...
namespace Almanna {
	public class GenerateSchema : Object {
		...
		public bool subclasses { get; set; default = false; }
		...
		private void generate_loader() {
			...
			foreach ( var entity in entities ) {
				lines.add(
					"Repo.add_entity( typeof(%s%s%s) );".printf(
		...
		private void generate_entity( string table_name, ArrayList<FoundColumn> columns, ArrayList<string> primary_keys ) {
			...
			if ( namespace != null ) {
				lines.insert( 0, "namespace %s%s {".printf( namespace, ( subclasses ? ".Entity" : "" ) ) );
				lines.add("}");
			}
			...
			string path = "%s%s.vala".printf( ( subclasses ? "Entity/" : "" ), class_name );
		...
		private void generate_subclass_for_entity( string class_name ) {
			...
			lines.add( " * Generated by almanna-generate.");
			lines.add( " */");
			lines.add( "public class %s : %sEntity {".printf( class_name, ( namespace != null ? namespace + "." : "" ) ) );
			lines.add("}");

			// Prepend namespace, if given
			if ( namespace != null ) {
				lines.insert( 0, "namespace %sImplementation {".printf( namespace != null ? namespace + "." : "" ) );
				lines.add("}");
			}
			...
			create_file_from_lines( "Implementation/%s.vala".printf(class_name), lines );
		}

		private void create_file_from_lines( string file_path, ArrayList<string> lines ) {
			try {
